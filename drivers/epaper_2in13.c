...
{
	GPIO_PortClear(BOARD_DP_DC_GPIO, 1u << BOARD_DP_DC_PIN);
}
...
{
	GPIO_PortSet(BOARD_DP_DC_GPIO, 1u << BOARD_DP_DC_PIN);
}
...
void EPD_Reset(void)
{
	...
	GPIO_PortSet(BOARD_DP_RST_GPIO, 1u << BOARD_DP_RST_PIN);
	GPIO_PortClear(BOARD_DP_RST_GPIO, 1u << BOARD_DP_RST_PIN);
	GPIO_PortSet(BOARD_DP_RST_GPIO, 1u << BOARD_DP_RST_PIN);
}
...
{
	PRINTF("e-Paper busy\r\n");
	while(GPIO_PinRead(BOARD_DP_BUSY_GPIO, BOARD_DP_BUSY_PIN) == 1) {      //LOW: idle, HIGH: busy
		//DEV_Delay_ms(100);
	}
